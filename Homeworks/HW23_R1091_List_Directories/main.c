...
int main(int argc, char *args[]) {
    ...
    int index, *returnValue, *removedSpaces, movedFiles = 0;
    ...
    if (argc == 2) {
        ...
        if (dir) {
            ...
            for (index = 0; index < movedFiles; ++index) {
                pthread_join(threadIds[index], (void **)&returnValue);
                *(removedSpaces + index) = *returnValue;
            }
            printf("Arhchivos movidos: %d\n", movedFiles); // Meterle esto por thread
            for (index = 0; index < movedFiles; index++)
                printf("En el archivo %d: se eliminaron: %d espacios en blanco\n", index, removedSpaces[index]);
        }
    }
